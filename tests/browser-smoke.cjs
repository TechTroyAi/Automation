...
(async () => {
  ...
  try {
    ...
    const firstLine = page.locator('.quote-line').first();
    await firstLine.locator('[name="quantity"]').fill('2');
    await firstLine.locator('[name="pearls"]').fill('2');
    await page.locator('#quoteFulfillment').selectOption('delivery');
    await page.locator('#quoteCalculate').click();
    assert.match(await page.locator('#quoteResult').innerText(), /TOTAL: ₱230.00/);
    await page.locator('#quotePayment').click();
    assert.match(await page.locator('#quotePaymentText').innerText(), /GCASH-TEST-000/);
    assert.match(await page.locator('#quotePaymentText').innerText(), /DO NOT SEND MONEY/);
    assert.match(await page.locator('#quotePaymentText').innerText(), /Sample amount: ₱230.00/);
    await firstLine.locator('[name="quantity"]').fill('3');
    assert.equal(await page.locator('#quotePayment').isDisabled(), true);
    assert.equal(await page.locator('#quotePaymentText').isHidden(), true);
    assert.doesNotMatch(await page.locator('#quoteResult').innerText(), /TOTAL:/);
    // Per-cup topping count cannot exceed quantity, even though both inputs are individually valid.
    await firstLine.locator('[name="pearls"]').fill('4');
    await page.locator('#quoteCalculate').click();
    assert.match(await page.locator('#quoteResult').innerText(), /Pearl add-ons/);
    assert.equal(await page.locator('#quotePayment').isDisabled(), true);
    await page.locator('#quoteClear').click();
    await firstLine.locator('[name="product"]').selectOption('okinawa');
    await firstLine.locator('[name="quantity"]').fill('5');
    await page.locator('#quoteAdd').click();
    await page.locator('#quoteFulfillment').selectOption('delivery');
    await page.locator('#quoteCalculate').click();
    assert.match(await page.locator('#quoteResult').innerText(), /Delivery: ₱0.00/);
    assert.match(await page.locator('#quoteResult').innerText(), /TOTAL: ₱500.00/);
    await page.locator('.quote-remove').last().click();
    assert.equal(await page.locator('.quote-line').count(), 1);
    assert.equal(await page.locator('#quotePayment').isDisabled(), true);
    await page.locator('#quoteClear').click();
    assert.equal(await firstLine.locator('[name="quantity"]').inputValue(), '1');
    assert.equal(await page.locator('#quoteFulfillment').inputValue(), 'pickup');
    ...
    await page.locator('#quoteCalculate').click();
    assert.match(await page.locator('#quoteResult').innerText(), /TOTAL: ₱75.00/);
    ...
    assert.match(await missingCalculator.locator('#quoteResult').innerText(), /calculator could not load/);
    assert.equal(await missingCalculator.locator('#quoteCalculate').isDisabled(), true);
    ...
    await local.locator('#quoteCalculate').click();
    assert.match(await local.locator('#quoteResult').innerText(), /TOTAL: ₱75.00/);
    await offline.close();
